...
| `survival.ts` | ⚠️ **provisional** survival interface (Phase 5 calibrates market/ADP). `P(player survives)`, `P(tier survives)`, confidence, fallback |
...
## Provisional / deferred to Phase 5

The **survival interface** is the weak point. Bloodline has no ADP pack, so
survival is derived from Sleeper `search_rank` as an ordinal ADP proxy
(`confidence: LOW`). The interface is clean (source / confidence / timestamp /
fallback) and the engine behaves at every confidence level, but the numbers are
not calibrated. Phase 5 owns real market/ADP/survival modelling.

## Reproduce

```bash
npx tsx scripts/phase4-harness.ts                 # scenarios, ablation, weights, monotonicity, latency
Rscript analysis/phase4_snake_recommendation_engine.R   # analysis + plots
node --test --import tsx test/draft-recommendation.test.ts
```
