...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    description:
      "READ-ONLY analytical retrieval of structured EvidenceBlocks (Phase 3.5C). ?capabilities=1 lists topics, required parameters and per-surface Book-Ready capability (current / history / comparison / decomposition / confidence / freshness / lineage / units / chart-readiness) and the refresh status of each artifact. ?topic=<fi.team_metric|fi.player_usage|role.player_profile|role.role_change|opp.scenario|scheme.qb_progression|scheme.qb_spatial|scheme.qb_formation|scheme.defense_coverage|startsit.shadow|matchup.shadow|waiver.status|waiver2.actions|waiver2.market|waiver2.replacement|roster_health.team|schedule_planning.team|trade.evaluation>. Source-native values are preserved next to a normalized analysis class; confidence, sample support, freshness, deployment and predictive status are INDEPENDENT axes; units/scale are explicit per metric (values above 1 can be legitimate); every comparison states its population; history comes only from immutable published snapshots and is labelled; Opportunity Propagation is CONDITIONAL, never an unconditional projection; SHADOW and DESCRIPTIVE evidence is never presented as production-predictive. history=1 and comparisons=1 are opt-in. Never writes prose or changes any production recommendation; SHADOW model outputs (Start/Sit, matchup, Waiver 2.0) are exposed as SHADOW evidence, never as production output.",
    scope: "service",
  ...
  },
  {
    ...
    description:
      "POST endpoint. SEPARATE from legacy mutual-benefit trade discovery: this optimizes the requesting manager's championship equity subject to the deal staying plausibly acceptable to the counterparty — the other manager's improvement is a cost, not an objective. Modes: 'evaluate' (one explicit proposal via give_assets / receive_assets / counterparty), 'negotiate' (base deal plus value-extraction / negotiation envelope), 'discover' (search for analytically positive direct trade candidates for the requester), 'strategy_path' (direct vs hold vs buy-and-hold vs bounded two-step vs no-action). Every trade carries BOTH a competitive_classification (STRONG_COMPETITIVE_BUY … REJECT — is it analytically attractive) and a transaction_readiness (NOT_RECOMMENDED / REVIEW_REQUIRED / EXPLORATORY / NEGOTIATION_WORTH_EXPLORING / TRANSACTION_READY — should you act on it now); a positive classification with LOW confidence or LOW acceptance is NEVER TRANSACTION_READY. discover may return analytically positive candidates while still recommending NO_ACTION because none is transaction-ready — that is not a contradiction. status NO_ACTION is a SUCCESSFUL result (HTTP 200), not an error or a not-ready state; status REVIEW_REQUIRED is preserved and never softened into a recommendation; confidence (VERY_LOW/LOW/MEDIUM/HIGH) and readiness are passed through, never upgraded. Opponent-threat calculations use exactly the roster identities in the current snapshot (co-managed teams are one participant, not two); a participant-set mismatch fails the competitive recommendation closed. Every response carries the source snapshot lineage (league_snapshot_id, generated_at, scoring/roster fingerprints, player-data version). Acceptance likelihood is heuristic (one real league trade exists); opponent threat is heuristic; market appreciation potential is speculative; early-season dynamic market evidence is preseason-only; multi-step trade paths are bounded to a maximum of two completed trades. READ-ONLY analytics — it never submits, accepts, or modifies a trade and never sends a message.",
    scope: "league",
