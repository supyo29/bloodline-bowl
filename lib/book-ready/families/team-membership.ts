...
export function teamMembershipEvidence(r: TeamResolution, o: { opponent?: OpponentResolution | null; scheme_as_of_team?: string | null; sources?: SourceState[]; identity_unresolved?: boolean } = {}): EvidenceBlock[] {
  const down = (o.sources ?? []).filter((x) => x.state !== "OK"); const srcNote = down.length ? [`TEMPORAL_SOURCE_UNAVAILABLE: ${down.map((d) => `${d.source} ${d.state}${d.detail ? ` (${d.detail})` : ""}`).join("; ")} — evidence from these sources is absent, which is different from evidence of absence`] : []; const code = o.identity_unresolved ? "IDENTITY_UNRESOLVED" : r.failure_code === "MEMBERSHIP_UNKNOWN" && down.length ? "TEMPORAL_SOURCE_UNAVAILABLE" : r.failure_code;
  ...
  const base = { surface: MEMBERSHIP_SURFACE, topic: "player.team_membership", subject: { kind: "PLAYER" as const, id: r.gsis_id }, deployment: { state: "SHARED_DESCRIPTIVE" as const, may_influence_production: false }, temporal,
    freshness: { as_of: null, through_week: null, generated_at: null }, lineage: { surface_version: TEMPORAL_IDENTITY_VERSION, content_identity: `tm:${identity}`, canonical: { policy: r.policy, evidence_ids: r.evidence_ids, temporal_data_version: r.evidence_version, source_vintages: r.source_vintages, basis: r.basis }, depends_on: [] as EvidenceBlock["lineage"]["depends_on"] },
  ...
  if (o.scheme_as_of_team !== undefined) { const f = schemeVintageFlag(o.scheme_as_of_team, resolved ? r.team : null); out.push(mkRaw({ ...base, metric: "membership.scheme_window_vintage", availability: { state: "AVAILABLE" }, value: f, unit: unitFor("category"), category: { raw: f, normalized: f, mapping_status: "VERIFIED" }, limitations: [...base.limitations, "Player-Scheme's `current_team` window = career rows on the AS-OF team (his last game-level team at the Player-Scheme cutoff, 2025 week 18), NOT necessarily the current club; the Player-Scheme directory's `nfl_team` is a DIFFERENT, current-season label and is not the window's team."] }, ["scheme"])); }
  return out;
}
