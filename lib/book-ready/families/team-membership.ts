...
export function teamMembershipEvidence(r: TeamResolution, o: { opponent?: OpponentResolution | null; scheme_as_of_team?: string | null } = {}): EvidenceBlock[] {
  ...
  const base = { surface: MEMBERSHIP_SURFACE, topic: "player.team_membership", subject: { kind: "PLAYER" as const, id: r.gsis_id }, deployment: { state: "SHARED_DESCRIPTIVE" as const, may_influence_production: false }, temporal,
    freshness: { as_of: null, through_week: null, generated_at: null }, lineage: { surface_version: TEMPORAL_IDENTITY_VERSION, content_identity: `tm:${identity}`, canonical: { policy: r.policy, evidence_ids: r.evidence_ids }, depends_on: [] as EvidenceBlock["lineage"]["depends_on"] },
  ...
  const comps: Component[] = [cat("status", r.status), cat("granularity", r.granularity ?? "NONE"), cat("evidence_class", r.evidence_class ?? "NONE"), ...(r.gap_weeks != null ? [cat("gap_weeks", r.gap_weeks, "missing weeks between the bracketing observations")] : []), ...cands];
  out.push(mkRaw(resolved
    ? { ...base, metric: "membership.team", availability: { state: "AVAILABLE" }, value: r.team!, unit: unitFor("category"), category: { raw: r.team!, normalized: r.team!, mapping_status: "VERIFIED" }, components: comps, limitations: [...base.limitations, ...r.reasons, ...(r.evidence_class === "SNAPSHOT_STALE_RISK" ? ["identity-table snapshot: may be stale"] : [])] }
    : { ...base, metric: "membership.team", availability: { state: "UNAVAILABLE", reason: `${r.status}: ${r.reasons.join(" ")}` }, origin: { source_class: "UNAVAILABLE", analysis_class: "UNAVAILABLE" }, limitations: [...base.limitations, ...r.reasons, ...(r.candidates.length ? [`candidates: ${r.candidates.map((c) => `${c.team ?? "no team"} [${c.sources.join("+")}${c.weeks.length ? ` weeks ${c.weeks.join(",")}` : ""}]`).join(" | ")}${r.selected_by_policy ? `; policy names ${r.selected_by_policy} for lineage only` : ""}`] : [])] } as never, ["team"]));
