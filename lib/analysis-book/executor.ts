...
export async function executePlan(plan: ResearchPlan, o: { client?: EvidenceClient; cache?: QueryCache; snap?: CapabilitySnapshot; concurrency?: number } = {}): Promise<Execution> {
  ...
  const run = async (q: PlannedQuery): Promise<void> => {
    if (q.deferred) { results.set(q.key, { key: q.key, topic: q.topic, status: "DEFERRED", detail: q.deferred, refs: [], identities: [], available: 0, total: 0, limitations: [q.deferred], ms: 0, bytes: 0, from_cache: false, history: q.history, comparisons: q.comparisons }); return; }
    ...
    try {
      ...
      const res: QueryResult = { key: q.key, topic: q.topic, status: ok ? "OK" : "ERROR", detail: ok ? null : `${r.status}${r.detail ? `: ${r.detail}` : ""}`, refs, identities: identitiesOf(refs), available: refs.filter((x) => x.availability === "AVAILABLE").length, total: refs.length, limitations: ok ? lim : [r.detail ?? r.status], ms: Math.round(performance.now() - t), bytes: r.performance?.approx_bytes ?? 0, from_cache: false, history: q.history, comparisons: q.comparisons };
      results.set(q.key, res); if (ok) cache.set(res);
    } catch (e) { results.set(q.key, { key: q.key, topic: q.topic, status: "ERROR", detail: (e as Error).message, refs: [], identities: [], available: 0, total: 0, limitations: [(e as Error).message], ms: Math.round(performance.now() - t), bytes: 0, from_cache: false, history: q.history, comparisons: q.comparisons }); }
  };
