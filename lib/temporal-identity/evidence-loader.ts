...
export interface MembershipEvidence { observations: TeamObservation[]; sources: SourceStatus[]; /** whether the player id exists in the identity table (null = could not be checked) */ identity_known: boolean | null }
...
export async function loadMembershipEvidence(o: { gsis_id: string; season: number; sleeper_id?: string | null; current: boolean; root?: string }): Promise<MembershipEvidence> {
  const observations: TeamObservation[] = []; const sources: SourceStatus[] = []; let identity_known: boolean | null = null;
  ...
  try {
    ...
    if (!rest) sources.push({ source: "game_logs", state: "NOT_CONFIGURED", detail: "Supabase is not configured in this environment" });
      ...
      identity_known = row.length > 0; if (row[0]) observations.push(crosswalkSnapshot(o.gsis_id, o.season, row[0].latest_team, "nfl_players@2026-03-18")); sources.push({ source: "identity_snapshot", state: "OK" });
    }
  ...
  return { observations, sources, identity_known };
}
