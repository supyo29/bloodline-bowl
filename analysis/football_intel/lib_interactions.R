...
build_receiver_progression <- function(ftn, pbp, ff_playerids, season, through_week, FI) {
  ...
  plays <- pbp %>%
    ...
    transmute(
      ...
      receiving_yards = coalesce(yards_gained, 0),
      air_yards, yac = ifelse(complete_pass == 1, yards_after_catch, NA_real_),
  ...
  out <- joined %>%
    ...
    summarise(
      ...
      yards_per_target = mean(receiving_yards, na.rm = TRUE),
      ...
      explosive_rate = mean(complete == 1 & receiving_yards >= FI$EXPLOSIVE_PASS_YARDS, na.rm = TRUE),
