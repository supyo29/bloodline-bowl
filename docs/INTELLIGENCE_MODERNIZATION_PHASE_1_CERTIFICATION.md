...
## 0. Merge reconciliation addendum

Between certification and merge, `origin/main` advanced by one commit:
`76ee903` (`github-actions[bot]`, the daily Football Intelligence refresh —
`fi:2026:w01:ab802a853780 -> fi:2026:w01:890f7aefd53f`). Classified as a
pure, non-overlapping automated data refresh: same season (2026), same
`through_week` (1), same `week_completion` shape (`COMPLETE`, 16/16) — only
served numeric values and the content hash changed (a routine upstream
correction), touching only
`lib/football-intel/data/{football_intelligence_manifest.json,ftn_descriptive.csv,team_profile.csv}`.
Zero overlap with any Phase 1 code path. Reconciled via a merge commit
(`2395522`) into the certified branch; every certification gate was
re-run against the reconciled state (all green — see §8/§9, updated
below) before merging to `main`. No Phase 1 contract, test, or behavior
...
Original certification (`fi:2026:w01:ab802a853780`):
```
...
**Re-run against the merge-reconciled state** (`fi:2026:w01:890f7aefd53f`,
per §0 — required because the reconciled drift touched FI artifacts
directly, not merely referenced from the prior result):
```
Rscript analysis/football_intel/tests/run.R        -> 24 invariant assertions, 0 failed
Rscript analysis/football_intel/adversarial_audit.R -> 15/15 checks PASS
Rscript analysis/football_intel/validate_snapshot.R lib/football-intel/data lib/football-intel/data
                                                     -> 30/30 gates PASS (self-comparison)
buildFootballIntelligenceLineage() + loadNflRealityFrontier(2026) +
  assessIntelligenceFreshness(): manifest reads correctly, week_completion
  parses correctly, overall_status = CURRENT -- confirmed live
```

No FI snapshot was published or regenerated by Phase 1 work itself — the
version change came solely from reconciling the daily-refresh automation's
own commit (§0).
...
Re-run against the merge-reconciled state (merge commit `2395522`,
`origin/main` `76ee903` reconciled in):
```
...
Itemized re-verification: FI reader/contract + intelligence-freshness +
Checkpoint C consumer-integration + Start/Sit-FI + orchestrator/system-trust
(131/131), waiver + Start/Sit-FI + matchup + Matchup Intelligence (83/83),
trade boundary + trade engine (25/25) — all green.
...
**Frozen as of merge to `main`** (see §14 for the merge SHA once recorded):

```text
RecommendationLineage
FootballIntelligenceLineage
NFL Reality Frontier
Intelligence Freshness Policy   (freshness-policy:2026.2)
Recommendation Readiness
Deployment-permission separation
```

**Future intelligence phases (Phase 2 — Player Role & Opportunity
Intelligence — onward) must consume these contracts rather than
implementing their own freshness or lineage interpretation.** This freeze
covers the *interfaces and ownership boundaries* above, not their internal
implementation details — feature-family lists, reason-code unions, operation
types, and disagreement-classification thresholds may all still be extended
additively by later phases; what may not happen is a second, independent
freshness/lineage system growing up alongside these.
