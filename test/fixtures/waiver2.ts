...
export function mkWaiverInput(f: WaiverFixture): WaiverInput {
  ...
  const weekly = weeklyContext({ week, myRoster, oppRoster, players: [...mine, ...others].map(mkPlayer), projections: [...mine, ...others].map(mkProj), freeAgents: f.freeAgents.map(mkPlayer), faProjections: f.freeAgents.map(mkProj), constraints: f.constraints, raw_scoring: f.raw_scoring, waiver_settings: f.waiver_type === "rolling" ? { type: "rolling", faab_budget: null, waiver_day: null } : f.waiver_type === "free_agency" ? { type: "unknown", faab_budget: null, waiver_day: null } : { type: "faab", faab_budget: 100, waiver_day: null } });
