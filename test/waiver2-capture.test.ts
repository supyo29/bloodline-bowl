...
test("EVIDENCE GATE: thresholds are fixed up front (not tiny); only LIVE_CAPTURED + certified + verified + complete + outcome counts; nothing is eligible today; the gate never promotes", () => {
  ...
  for (let w = 0; w < 8; w++) for (const mgr of ["a", "b", "c"]) for (let k = 0; k < 3; k++) { const input = fx({ week: 2 + w, freeAgents: [{ id: "fa1", pos: "WR", pts: 11 + k * 0.1, team: "CHI", ros: 150 }] }); const ev = evaluateWaiver2(input); const r = buildCaptureRecord(ev, input, { league_slug: "L", manager_slug: mgr, season: 2026, kind: "LIVE_CAPTURED", lock: PRE }); many.push(r); outs.push({ capture_id: r.capture_id }); }
  ...
  assert.equal(gate.eligible_records, 72); assert.equal(gate.distinct_weeks, 8); assert.equal(gate.status, "MINIMUM_MET_FOR_REVIEW"); assert.ok(gate.unmet.length > 0, "preferred not met"); assert.equal(gate.excluded.NOT_ACTIONABLE, 1); assert.ok((gate.excluded.LIVE_UNVERIFIED ?? 0) === 1); assert.equal(gate.lifecycle_state, "SHADOW_ONLY");
});
